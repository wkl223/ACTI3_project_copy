...
<%@ attribute name="noResponsePanel" type="teammates.ui.template.InstructorFeedbackResultsNoResponsePanel"
              required="true" %>
<c:choose>
      ...
      <table class="table table-striped table-bordered margin-0">
        ...
        <c:forEach items="${noResponsePanel.emails}" var="email">
          <tr>
            <c:choose>
              <c:when
                  test="${not empty noResponsePanel.instructorStatus[email] && noResponsePanel.instructorStatus[email] == 'true'}">

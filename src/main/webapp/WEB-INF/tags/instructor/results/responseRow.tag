...
<tr
<c:forEach items="${responseRow.rowAttributes.attributes}" var="attr">${attr.key}="${attr.value}"</c:forEach>>
...
<c:if test="${responseRow.recipientDisplayed}">
  ...
  <td class="word-wrap-break middlealign<c:if test="${responseRow.rowGrey}"> color-neutral</c:if>">
    <c:choose>
      <c:when
          test="${not empty responseRow.recipientProfilePictureLink && !responseRow.recipientProfilePictureAColumn}">
  ...
  <td>
    ...
    <c:if test="${not responseRow.rowGrey && responseRow.instructorCommentsOnResponsesAllowed}">
      <button type="button" class="btn btn-default btn-xs comment-button" style="margin-top:0.5em;"
              data-toggle="modal"
              data-target="#commentModal-${responseRow.responseRecipientIndex}-${responseRow.responseGiverIndex}-${questionIndex}"
              data-recipientindex="${responseRow.responseRecipientIndex}"
              data-giverindex="${responseRow.responseGiverIndex}"
              data-qnindex="${questionIndex}">
        Add Comment
      </button>
      <results:commentModal responseRow="${responseRow}" responseRecipientIndex="${responseRow.responseRecipientIndex}"
                            responseGiverIndex="${responseRow.responseGiverIndex}"

...
<%@ attribute name="groupByParticipantPanel"
    ...
    <c:choose>
      <c:when test="${groupByParticipantPanel.emailValid}">
        <div class="middlealign profile-pic-icon-hover inline panel-heading-text"
             data-link="${groupByParticipantPanel.profilePictureLink}">
          <strong>${fn:escapeXml(groupByParticipantPanel.name)}</strong>
          <img src="" alt="No Image Given" class="hidden profile-pic-icon-hidden">
          <a
              <c:if
                  test="${not empty groupByParticipantPanel.secondaryParticipantPanels}">class="link-in-dark-bg"</c:if>
              href="mailto:${groupByParticipantPanel.participantIdentifier}">[${groupByParticipantPanel.participantIdentifier}]</a>
        </div>
      </c:when>
      <c:otherwise>
  ...
  <div class="panel-collapse collapse in">
    <div class="panel-body">
      <c:choose>
        ...
        <c:otherwise>
          <i>There are no responses ${groupByParticipantPanel.giver? 'given' : 'received'} by this user or you may not
            have the permission to see the response</i>
        </c:otherwise>
      </c:choose>
    </div>
  </div>

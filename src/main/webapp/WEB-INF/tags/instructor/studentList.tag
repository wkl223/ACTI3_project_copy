...
<table
    class="table table-bordered table-striped<c:if test="${not fromCourseDetailsPage}"> table-responsive margin-0</c:if>">
  <c:choose>
    <c:when test="${not empty sections}">
      <thead class="${tableHeaderClass}">
      <tr id="resultsHeader-${courseIndex}">
        <th>Photo</th>
        <th id="button_sortsection-${courseIndex}"
            class="toggle-sort button-sort-none<c:if test="${not hasSection}"> hidden</c:if>">
        ...
        <c:forEach items="${section.teams}" var="team">
          ...
          <c:if test="${fromStudentListPage}">
            ...
            <div class="checkbox team-to-be-transported">
              <input id="team_check-${courseIndex}-${sectionIndex}-${teamIndex}" type="checkbox" checked
                     class="team_check">
          ...
          <c:forEach items="${team.students}" var="student" varStatus="studentIdx">
            ...
            <tr class="student_row" id="student-c${courseIndex}.${studentIndex}">
              ...
              <td class="no-print align-center">
                ...
                <c:if
                    test="${(fromCourseDetailsPage || fromStudentListPage) && student.studentStatus == STUDENT_COURSE_STATUS_YET_TO_JOIN}">

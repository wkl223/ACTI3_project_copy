...
<div class="col-sm-12 padding-15px margin-bottom-15px background-color-light-green">
  ...
  <div class="feedback-path-dropdown btn-group">
    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"
      ...
      <c:choose>
        ...
        <c:when test="${fqForm.feedbackPathSettings.commonPathSelected}">
          ${fqForm.feedbackPathSettings.selectedGiver.displayNameGiver} will give feedback on <span
            class='glyphicon glyphicon-arrow-right'></span> ${fqForm.feedbackPathSettings.selectedRecipient.displayNameRecipient}
        </c:when>
        <c:otherwise>
    ...
    <ul class="dropdown-menu">
      ...
      <c:forEach items="<%= Const.FeedbackQuestion.COMMON_FEEDBACK_PATHS %>" var="commonPath">
        <li class="dropdown-submenu">
          ...
          <ul class="dropdown-menu" data-toggle="tooltip" data-trigger="manual" data-placement="top"
              data-container="body" title="Choose an option">
            <li>
      ...
      <li><a class="feedback-path-dropdown-option feedback-path-dropdown-option-other" href="javascript:;"
             data-path-description="Predefined combinations:">Other predefined combinations...</a></li>
    </ul>
  </div>
  <div class="feedback-path-others margin-top-7px"<c:if
      test="${fqForm.feedbackPathSettings.commonPathSelected || isNewQuestion}"> style="display:none;"</c:if>>
      ...
      <div class="col-sm-8 col-lg-7">
        <select class="form-control participantSelect"
          ...
          <c:forEach items="<%= FeedbackParticipantType.GIVERS %>" var="giverType">
            <option <c:if test="${fqForm.feedbackPathSettings.selectedGiver eq giverType}">selected
                    </c:if>value="${giverType}">
      ...
      <div class="col-sm-8 col-lg-7">
        <select class="form-control participantSelect"
          ...
          <c:forEach items="<%= FeedbackParticipantType.RECIPIENTS %>" var="recipientType">
            <option <c:if test="${fqForm.feedbackPathSettings.selectedRecipient eq recipientType}">selected
                    </c:if>value="${recipientType}">
    ...
    <div class="col-sm-12 row numberOfEntitiesElements">
      <label class="control-label col-sm-4 small">
        The maximum number of <span class='number-of-entities-inner-text'></span> each respondent should give feedback
        to:
      </label>

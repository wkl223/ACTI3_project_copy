...
<c:set var="FEEDBACK_RECEIVER"><%=FeedbackParticipantType.RECEIVER.name()%>
</c:set>
<c:set var="FEEDBACK_OWN_TEAM_MEMBERS"><%=FeedbackParticipantType.OWN_TEAM_MEMBERS.name()%>
</c:set>
<c:set var="FEEDBACK_RECEIVER_TEAM_MEMBERS"><%=FeedbackParticipantType.RECEIVER_TEAM_MEMBERS.name()%>
</c:set>
<c:set var="FEEDBACK_STUDENTS"><%=FeedbackParticipantType.STUDENTS.name()%>
</c:set>
<c:set var="FEEDBACK_INSTRUCTORS"><%=FeedbackParticipantType.INSTRUCTORS.name()%>
</c:set>

<div
    class="col-sm-12 margin-bottom-15px padding-15px <%= fqForm.isQuestionHasResponses() ? "alert alert-danger" : "background-color-light-green" %>">
  <div class="margin-bottom-7px">
    <c:if test="${fqForm.questionHasResponses}">
      ...
      <p>Reason: The existing responses were submitted under the 'promise' of a certain visibility and changing the
        visibility later 'breaks' that promise.</p>
      <br/>
    </c:if>
      ...
      <c:forEach items="<%= Const.FeedbackQuestion.COMMON_VISIBILITY_OPTIONS %>" var="visibilityOption">
        <li>
          <a class="visibility-options-dropdown-option" href="javascript:;"
             data-option-name="${visibilityOption.key}">${visibilityOption.value}</a>
        </li>
      </c:forEach>
      <li role="separator" class="divider"></li>
      <li><a class="visibility-options-dropdown-option" href="javascript:;" data-option-name="OTHER">Custom visibility
        options...</a></li>
    </ul>
  </div>
      ...
      <tr>
        ...
        <td>
          <input class="visibilityCheckbox answerCheckbox"
                 type="checkbox" value="<%= FeedbackParticipantType.OWN_TEAM_MEMBERS %>"
                 <c:if test="${!fqForm.editable}">disabled</c:if>
        ...
        <td>
          <input class="visibilityCheckbox recipientCheckbox"
                 type="checkbox" value="<%= FeedbackParticipantType.OWN_TEAM_MEMBERS %>"
                 <c:if test="${!fqForm.editable}">disabled</c:if>
          <c:if test="${fqForm.visibilitySettings.recipientNameVisibleFor[FEEDBACK_OWN_TEAM_MEMBERS]}">
                 checked=""</c:if> >
        </td>
      </tr>
      <tr>
        ...
        <td>
          <input class="visibilityCheckbox answerCheckbox" type="checkbox"
                 ...
                 <c:if test="${!fqForm.editable}">disabled</c:if>
          <c:if test="${fqForm.visibilitySettings.responseVisibleFor[FEEDBACK_RECEIVER_TEAM_MEMBERS]}">
                 checked=""</c:if> >
        </td>
        <td>
          <input class="visibilityCheckbox giverCheckbox" type="checkbox"
                 value="<%= FeedbackParticipantType.RECEIVER_TEAM_MEMBERS %>"
                 <c:if test="${!fqForm.editable}">disabled</c:if>
          <c:if test="${fqForm.visibilitySettings.giverNameVisibleFor[FEEDBACK_RECEIVER_TEAM_MEMBERS]}">
                 checked=""</c:if> >
        </td>
        <td>
          <input class="visibilityCheckbox recipientCheckbox" type="checkbox"
                 value="<%= FeedbackParticipantType.RECEIVER_TEAM_MEMBERS %>"
                 <c:if test="${!fqForm.editable}">disabled</c:if>
          <c:if test="${fqForm.visibilitySettings.recipientNameVisibleFor[FEEDBACK_RECEIVER_TEAM_MEMBERS]}">
                 checked=""</c:if> >
        </td>
      </tr>
      <tr>

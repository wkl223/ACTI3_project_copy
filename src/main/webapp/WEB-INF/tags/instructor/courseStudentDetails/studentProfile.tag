...
<c:set var="none"><i class="text-muted"><%= Const.STUDENT_PROFILE_FIELD_NOT_FILLED %>
</i></c:set>
<c:set var="noneForGender"><span class="text-muted"><%= Const.STUDENT_PROFILE_FIELD_NOT_FILLED %></span></c:set>
<c:set var="other"><%= Const.GenderTypes.OTHER %>
</c:set>
<c:set var="jsIncludes">
  <script type="text/javascript" src="/js/studentLike.js"></script>
</c:set>
<div class="row">
  <div class="col-xs-12">
          ...
          <tr>
            ...
            <td>${empty student.like ? none : fn:escapeXml(student.like)}
              <form action="<%=Const.ActionURIs.INSTRUCTOR_COURSE_STUDENT_DETAILS_LIKE%>" method="get">
                ...
                <input type="hidden" name="<%= Const.ParamsNames.STUDENT_SHORT_NAME %>"
                       data-actual-value="<c:out value="${student.shortName}"/>"
                       value="<c:out value="${student.shortName}"/>">
                <input type="hidden" name="<%= Const.ParamsNames.STUDENT_EMAIL %>" value="${student.email}">
                <input type="hidden" name="<%= Const.ParamsNames.STUDENT_PROFILE_INSTITUTION %>"
                       data-actual-value="<c:out value="${student.institute}"/>"
                       value="<c:out value="${student.institute}"/>">

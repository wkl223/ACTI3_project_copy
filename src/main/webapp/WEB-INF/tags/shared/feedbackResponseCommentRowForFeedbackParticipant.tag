...
<li class="list-group-item list-group-item-warning"
    <c:if test="${not empty qnIndex && not empty responseIndex}">id="responseCommentRow-${divId}"</c:if>>
  <div
      <c:if test="${not empty qnIndex && not empty responseIndex}">id="commentBar-${divId}"</c:if> class="row">
    <div class="col-xs-10">
      ...
      <c:if test="${frc.editDeleteEnabled}">
        <div class="responseCommentDeleteForm pull-right float-right clearfix">
          ...
          <c:if test="${isModeration}">
            <input name="<%= Const.ParamsNames.FEEDBACK_SESSION_MODERATED_PERSON %>" value="${moderatedPersonEmail}"
                   type="hidden">
          </c:if>
        </div>
        <a type="button"
  ...
  <div
      <c:if test="${not empty qnIndex && not empty responseIndex}">id="plainCommentText-${divId}"</c:if>
      style="margin-left: 15px;">${frc.commentText}</div>
  <c:if test="${frc.editDeleteEnabled}">
    <c:set var="textAreaId"><%= Const.ParamsNames.FEEDBACK_RESPONSE_COMMENT_TEXT %>
    </c:set>

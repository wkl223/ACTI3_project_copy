...
<c:set var="isNumResponsesMax"
       value="${questionWithResponses.numOfResponseBoxes eq questionWithResponses.maxResponsesPossible}"/>
...
<c:set var="hasFeedbackParticipantCommentOnResponse"
       value="${not empty response.feedbackParticipantCommentOnResponse}"/>
...
<div class="evalueeForm-${questionWithResponses.question.qnIndx} form-group margin-0" style="padding-left: 75px">
  ...
  ${isRecipientNameHidden ?  'style="display:none"' : 'style="text-align:left"'}>
    ...
    <label>
      <select
          class="participantSelect middlealign<c:if test="${not response.existingResponse}"> newResponse</c:if> form-control"
      ...
      <ul class="list-group" id="responseCommentTable-${questionIndex}-${responseIndex}">
        <c:if test="${hasFeedbackParticipantCommentOnResponse}">
          <shared:feedbackResponseCommentRowForFeedbackParticipant
              frc="${response.feedbackParticipantCommentOnResponse}"
        ...
        <shared:feedbackResponseCommentAddFormForFeedbackParticipant frc="${response.feedbackResponseCommentAdd}"
                                                                     responseIndex="${response.responseIndx}"
                                                                     qnIndex="${questionWithResponses.question.qnIndx}"/>
      </ul>
    </div>
  </c:if>
